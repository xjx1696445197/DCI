...
<script>
    ...
    export default {
        ...
        mounted() {
            var that = this;
            this.userList = JSON.parse(localStorage.getItem("vaultlist"));
        ...
        methods: {
            ...
            getbalances(address){
                ...
                this.$http.get('http://120.77.247.234:8984/js/hCurrencyRate/findCurrencyParameter', {
                    ...
                    that.$http.get('/tokenManage/findTokenManage', {}).then((res) => {
                        that.Totalbalances=0
                        that.contractlist = res.data
                        for (var i = 0; i < res.data.length; i++) {
                            that.getBalance(address, res.data[i].contractPath,i).then((res) => {
                            })
            ...
            // 获取给定地址余额
            async getBalance(fromAddress, contractPath,index) {
                var that = this;
                ...
                await txu.balanceOf(web3, _from, contractPath).then((res) => {
                    that.Tokenbalance = res / 1000000000000000000
                    that.Totalbalances+=res / 1000000000000000000*that.usdtPrice.usdtCny
                })
            ...
            async getBalancelist() {
                ...
                for (var i = 0; i < that.userList.length; i++) {
                    ...
                    that.getbalances(that.userList[i].accounts)
