                            ...
                            <div class="sidebar_list_padding scroll">
                                <div class="sidebar_list">
                                    ...
                                    >
                                        ...
                                        <p class="sidebar_txt3"><span>{{accountlist[index]|addressFormat}}</span><img
                                                @click="initCopyer(item.accounts)"
                                                src="../static/images/newWallet/detail_copy_iconh.png"
                                                data-initcopyer="true"></p>
                                        <p class="sidebar_txt3"><span>￥{{contractPrice[index]+contractPrices[index]|tofixed4}}</span><img
                                                @click="showmb(index)"
...
<!--                                &lt;!&ndash;                                <p class="sidebar_txt2">{{$t('sidebar.sidebar_other')}}</p>&ndash;&gt;-->
    ...
    export default {
        ...
        mounted() {
            ...
            if (this.CurrentlyselectedWallet == 1) {
                ...
                // txu.balanceOf(that.web3, that.myaddress).then((res) => {
            ...
            // api_rootbalanceeth(that.myaddress)
            ...
            this.$http.get('http://120.77.247.234:8984/js/hCurrencyRate/findCurrencyParameter', {
                ...
                that.usdtCny = res.result.usdtCny
                that.chmcUsdt = res.result.chmcUsdt.firstPrice
                that.haleUsdt = res.result.haleUsdt.firstPrice
        ...
        methods: {
            ...
            getbalances(address){
                ...
                this.$http.get('http://120.77.247.234:8984/js/hCurrencyRate/findCurrencyParameter', {
                    ...
                    jsonAjax.get(urlUtil.getApiUrl("findTokenManage"), {
                        ...
                        that.Totalbalances=0
                        that.ethRmb=res.resultData.ethPrice
                        that.contractlist = res.resultData.tokenManage
                        for (var i = 0; i < res.resultData.tokenManage.length; i++) {
                            that.getBalancet(address, res.resultData.tokenManage[i].contractPath,i,res.resultData.tokenManage[i].quotes).then((res) => {
                            })

                            // that.getBalance(localStorage.getItem("assetaddress"),res.data[i].contractPath))
                        }
                        if (that.CurrentlyselectedWallet == 1) {
                            that.web3.eth.getBalance(address, function (err, value) {
                                ...
                                that.contractPrice.push(that.Totalbalance)
                            ...
                            that.web3.eth.getBalance(address, function (err, value) {
                                ...
                                that.contractPrice.push(that.Totalbalance)
                                console.log(value ,that.usdtPrice.usdtCny,that.ethRmb)
                                // console.log('-------getBalance-------err--------' + err)
                                console.log('-------getBalance--------value-------' + value)
                                if (err) {
                                    that.balance = ''
            ...
            // 获取给定地址余额
            async getBalancet(fromAddress, contractPath,index,quotes) {
                var that = this;
                ...
                await txu.balanceOf(web3, _from, contractPath).then((res) => {
                    if (that.CurrentlyselectedWallet == 1) {
                        that.Tokenbalance = res / 1000000000000000000
                        that.Totalbalances+=res / 1000000000000000000*that.usdtPrice.usdtCny*quotes
                    }else {
                        that.Tokenbalance = res / 1000000
                        that.Totalbalances+=res / 1000000*that.usdtPrice.usdtCny*quotes
                    }

                })
                if (index+1==that.contractlist.length){
                    that.contractPrices.push(that.Totalbalances)
                    console.log(that.contractPrices)
            ...
            async getBalancelist() {
                ...
                for (var i = 0; i < that.userList.length; i++) {
                    ...
                    that.getbalances(that.myaddress)
                    // await web3.eth.getBalance(that.userList[i].accounts, function (err, value) {
                    ...
                    // }.bind(that))
                    // // //代币chmc余额
                    // await txu.balanceOf(web3, that.userList[i].accounts).then((res) => {
                    //     that.balancetokenlist.push(res / 1000000000000000000)
                    // })
                    // // //代币edc余额
                    // await txu.balanceOfs(web3, that.userList[i].accounts).then((res) => {
                    //     that.balanceEdctokenlist.push(res / 1000000000000000000)
                    // })
                }
            },
            // // 获取给定地址余额
            ...
            checkwalletname() {
                ...
                if (this.CurrentlyselectedWallet == 1) {
                    localStorage.setItem("vaultlist", JSON.stringify(this.userList))
                    localStorage.setItem("walltname", this.walletname)
                } else {
                    localStorage.setItem("ethvaultlist", JSON.stringify(this.userList))
                    localStorage.setItem("ethwalltname", this.walletname)
                }
