        ...
        methods: {
            ...
            setAllRead(){
                ...
                jsonAjax.get(urlUtil.getApiUrl('markReadMsg'), {
                    ...
                    if( res.returnCode==1 ){
                        that.$refs.scrollView.triggerRefresh()
            ...
            getData(refresh){
                var that=this;
               return  jsonAjax.get(urlUtil.getApiUrl('getMsgPageListByUserId'), {
                    userId: this.userId,
                    msgType: this.type,
                    languageType: 1,
                    ...
                    if( res.returnCode==1){
                        if( refresh ){
                            that.list = res.resultData
                        } else {

                            that.list.push(...res.resultData)
                        }

                        that.$refs.scrollView.finishRefresh()
                        that.$refs.scrollView.finishLoadMore()
                        that.pageNo += 1

                        if( res.resultData.length < this.pageSize ){
                            that.isFinished = true
                            that.$refs.scrollView.loadend()
                        }
                    }
                })
               // this.$http.get('app/message/getMsgPageListByUserId', {
               //      userId: this.userId,
               //      msgType: this.type,
               //      languageType: 1,
               //      pageSize: this.pageSize,
