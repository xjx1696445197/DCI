...
class KeyringController extends EventEmitter {
    ...
    addNewKeyring(type, opts) {
        ...
        return keyring.getAccounts()
            ...
            .then(() => {
                ...
                return this.persistAllKeyrings("123")
            })
    ...
    exportAccountPublic(address) {
        try {
            return this.getKeyringForAccount(address)
                .then((keyring) => {
                    // return keyring.getPublicKey(normalizeAddress(address))
                    return keyring.exportAccountPubilc(normalizeAddress(address))
    ...
    getPublicKeyFor(address) {
        try {
            return this.getKeyringForAccount(address)
                .then((keyring) => {
                    return keyring.getPublicKeyFor(normalizeAddress(address))
                    // return keyring.getPublicKeyFor(normalizeAddress(address))
    ...
    getPrivateKeyFor(address) {
        try {
            return this.getKeyringForAccount(address)
                .then((keyring) => {
                    return keyring.getPrivateKeyFor(normalizeAddress(address))
                    // return keyring.getPublicKeyFor(normalizeAddress(address))
    ...
    persistAllKeyrings(password = this.password) {
        ...
        return Promise.all(this.keyrings.map((keyring) => {
            ...
            .then((encryptedString) => {
                //     vaultlist.push()
                localStorage.setItem("vault", JSON.stringify({"vault": encryptedString}))
    ...
    async unlockKeyrings(password) {
        ...
        // vaults.vault=JSON.parse()
        var vaul = {}
        if (localStorage.getItem("CurrentlyselectedWallet")==1){
            for (var i = 0; i < JSON.parse(localStorage.getItem("vaultlist")).length; i++) {
                if (JSON.parse(localStorage.getItem("vaultlist"))[i].active) {
                    vaul = JSON.parse(localStorage.getItem("vaultlist"))[i].vault
                } else {
                }
            }
        }else {
            for (var i = 0; i < JSON.parse(localStorage.getItem("ethvaultlist")).length; i++) {
                if (JSON.parse(localStorage.getItem("ethvaultlist"))[i].active) {
                    vaul = JSON.parse(localStorage.getItem("ethvaultlist"))[i].vault
                } else {
                }
            }
        }

        // const encryptedVault =JSON.parse(localStorage.getItem("vault")).vault
        const encryptedVault = vaul
        if (!encryptedVault) {
